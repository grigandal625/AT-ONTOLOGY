...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        agregation, _ = RelationshipType.objects.get_or_create(name="Агрегация", ontology_model=ontology_model)

        asociation, _ = RelationshipType.objects.get_or_create(name="Асоциация", ontology_model=ontology_model)

        weak, _ = RelationshipType.objects.get_or_create(name="Слабая", ontology_model=ontology_model)

        self.generate_relationships(vertices=insyst, ontology=insyst[0].ontology, types=[weak, asociation, agregation])

        self.generate_relationships(vertices=ids, ontology=ids[0].ontology, types=[weak, asociation, agregation])
        self.generate_relationships(vertices=dis, ontology=dis[0].ontology, types=[weak, asociation, agregation])

...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        ontology_3 = self.create_ontology(name="Динамические интеллектуальные системы")
        ontology_model = self.create_ontology_model(name="Прикладная онтология курса/дисциплины")

        element_course = self.create_vertex_type(
            name="Элемент курса/дисциплины",
            ontology_model=ontology_model
        )

        topic = self.create_vertex_type(
            name="Тема",
            ontology_model=ontology_model,
            derived_from=element_course
        )

        subtopic = self.create_vertex_type(
            name="Подтема",
            ontology_model=ontology_model,
            derived_from=element_course
        )
        ...
        for topic_id, text, parent_id, root_id, depth in rows:
            ...
            vertex, _ = Vertex.objects.get_or_create(
                name=text,
                ontology=ontology,
                type=vertex_type
            )
    ...
    def create_ontology(self, name: str) -> Ontology:
        ...
        if created:
            self.stdout.write(
                self.style.SUCCESS(f"Created Ontology: {ontology.name} (id={ontology.id})")
            )
        else:
    ...
    def create_ontology_model(self, name: str) -> OntologyModel:
        ...
        model, created = OntologyModel.objects.get_or_create(
            name=name
        )

        if created:
            self.stdout.write(self.style.SUCCESS(
                f"Created OntologyModel: {model.name} (id={model.id})"
            ))
        else:
            self.stdout.write(
                f"OntologyModel already exists: {model.name} (id={model.id})"
            )
        return model
    ...
    def create_hierarchical_relationship_type(self, ontology_model, topic_type, subtopic_type):
        rt, created = RelationshipType.objects.get_or_create(
            name="Иерархическая",
            ontology_model=ontology_model
        )

...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        weighted_relationship_type, created = RelationshipType.objects.get_or_create(
            name="Весовая", ontology_model=ontology_model
        )
        ...
        for competence_code, topic_text, weight in rows:
            try:
                ...
                self.stdout.write(
                    f"Выбрана первая вершина: {competence.name} " f"(онтология: {competence.ontology.name})"
                )
            try:
                ...
                self.stdout.write(f"Выбрана первая вершина: {topic.name} " f"(онтология: {topic.ontology.name})")

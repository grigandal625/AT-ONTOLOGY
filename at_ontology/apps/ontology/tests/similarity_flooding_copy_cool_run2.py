...
def _get_parent_label(vertex_id: str, graph: _Graph) -> str | None:
    ...
    Учитывает контекст вершины — «Основные понятия» под разными родителями
    должны получать разные оценки схожести.
    Если родителя нет — возвращает None.
    """
...
def _initial_sigma(
    ...
    σ⁰ = взвешенная комбинация:
      - схожесть label/name вершин (85%)
      - схожесть набора свойств (15%)
    Если у обеих вершин есть родители — учитываем их схожесть:
      - 70% своя схожесть + 30% схожесть родителей.
    Это позволяет различать вершины с одинаковым названием
    («Основные понятия и определения»), но разным контекстом.
    """
    ...
    if parent_a and parent_b:
        parent_sim = _string_sim(parent_a, parent_b)
...
def _normalize_dict(d: dict[PairKey, float]) -> dict[PairKey, float]:
    """Нормализация всех значений в [0..1] делением на максимум."""
    if not d:
        return d
...
class OntologySimilarityMatcher:
    ...
    (формула C, Melnik et al. 2002) с учётом контекста родительской вершины.
    ...
    def run(self, top_k: int | None = None) -> list[MatchResult]:
        ...
        print('Строим σ⁰ (с учётом контекста родителя)...')
        sigma0: dict[PairKey, float] = {}

...
# Ключ пары вершин: (name из онтологии A, name из онтологии B)
type PairKey = tuple[str, str]

@dataclass
class MatchResult:
    """Итоговое соответствие двух вершин."""
    vertex_a: object       # Vertex из онтологии A
    vertex_b: object       # Vertex из онтологии B
    score: float           # σ после нормализации [0..1]
    initial_score: float   # σ₀ до итераций (для отладки)


@dataclass
class _Edge:
    """Ребро внутри одной онтологии."""
    source_id: str
    target_id: str
    label: str
...
@dataclass
class _Graph:
    """Лёгкое представление онтологии как графа."""
...
class _PCGEdge:
    """
    Ребро в Pairwise Connectivity Graph.
    Направление: из пары (u_a, u_b) в пару (v_a, v_b),
    если в обоих исходных графах есть рёбра с одинаковой меткой.
    """
    from_pair: PairKey
    to_pair: PairKey
...
def _default_vertex_filter(vertex: object) -> bool:
    """
    Фильтр вершин по умолчанию — пропускает все вершины.

    Для реальных онтологий с Topic_*/Competence_* передайте свой фильтр,
    например: vertex_filter=lambda v: v.name.startswith("Topic_")
    """
    return True
...
def _get_rel_label(relationship: object) -> str:
    """
    Извлекает строковую метку типа связи.

    Поддерживает два варианта:
      1. Парсерный объект: relationship.type.alias = 'Pkg.relationship_types.Hierarchy'
         → берём последнюю часть ('Hierarchy')
      2. Простой объект с .type.name (для тестовых заглушек)
    """
...
def _get_vertex_from_rel_end(rel_end: object) -> object | None:
    """
    Извлекает объект вершины из конца связи.

    Парсерный объект: rel.source — OntologyReference с .value = Vertex
    Тестовая заглушка: rel.source — непосредственно объект вершины (с .name)
    """
...
def _load_graph(
    ...
    """
    Превращает объект Ontology из at_ontology_parser в лёгкий граф.

    ontology.vertices      — dict[str, Vertex]
    ontology.relationships — dict[str, Relationship]

    Каждый Relationship (парсерный) имеет:
      .source — OntologyReference с .value = Vertex
      .target — OntologyReference с .value = Vertex
      .type   — OntologyReference с .alias = 'Pkg.relationship_types.Hierarchy'

    vertex_filter — необязательная функция (vertex) -> bool.
    По умолчанию берутся все вершины.
    """
...
def _levenshtein(a: str, b: str) -> int:
    """
    Расстояние Левенштейна через динамическое программирование.
...
def _string_sim(a: str, b: str) -> float:
    """
    Нормализованная схожесть на основе расстояния Левенштейна [0..1].
    sim = 1 - lev(a, b) / max(len(a), len(b))
    """
    if not a or not b:
        return 0.0
    a, b = a.lower(), b.lower()
...
def _label_sim(v_a: object, v_b: object) -> float:
    """
    Схожесть двух Vertex по name и label.
    label (человеческое название) весит больше, чем name (системный slug).
    """
...
def _get_property_names(vertex: object) -> set[str]:
    """
    Возвращает множество имён свойств вершины.

    Поддерживает два варианта:
      1. Парсерный объект: vertex.properties — dict[str, ...]
         ключ словаря — имя свойства
      2. Django ORM: vertex.properties.select_related("definition").all()
         у каждого объекта есть .definition.name
    """
...
def _property_sim(v_a: object, v_b: object) -> float:
    """
    Jaccard-схожесть наборов свойств двух вершин [0..1].
    """
...
def _initial_sigma(v_a: object, v_b: object) -> float:
    """
    σ₀ = взвешенная комбинация схожести имён/label и свойств.
    """
...
def _build_pcg(graph_a: _Graph, graph_b: _Graph) -> list[_PCGEdge]:
    """
    Строим PCG: для каждой пары рёбер из A и B с одинаковой меткой
    добавляем ребро PCG (u_a, u_b) -> (v_a, v_b).

    Если совпадающих меток нет (тестовые онтологии без рёбер),
    PCG будет пустым — алгоритм корректно вернёт только σ₀.
    """
...
def _flood(
    ...
    """
    Итеративно пересчитываем σ по формуле Melnik et al.:

        σ_{t+1}(u, v) = σ_t(u, v)
                       + Σ  σ_t(x, y) / out_deg(x in A) / out_deg(y in B)
                         (x,y)->(u,v) ∈ PCG

    Если PCG пустой (нет рёбер) — итерации пропускаются, возвращаем σ₀.
    """
    if not pcg_edges:
...
def _normalize(sigma: dict[PairKey, float]) -> dict[PairKey, float]:
    """Нормализуем все σ в диапазон [0..1] делением на максимум."""
    if not sigma:
        return sigma
...
def _filter_one_to_one(
    ...
    """
    Фильтр «один к одному»: жадный алгоритм по убыванию σ.
    Каждая вершина из A и из B используется не более одного раза.
    """
...
class OntologySimilarityMatcher:
    ...
    iterations             : максимум итераций затопления (по умолчанию 20)
    min_score              : порог отсечения слабых совпадений (по умолчанию 0.05)
    vertex_filter          : (vertex) -> bool, фильтр вершин.
                             По умолчанию берутся все вершины.
    ...
    def run(self, top_k: int | None = None) -> list[MatchResult]:
        """
        Запустить алгоритм и вернуть список MatchResult,
        отсортированных по убыванию σ.
        """
        print(f'Загружаем граф «{self.ontology_a}»...')
        graph_a = _load_graph(self.ontology_a, self.vertex_filter)
        print(f'  {len(graph_a.vertices)} вершин, {len(graph_a.edges)} рёбер')

        print(f'Загружаем граф «{self.ontology_b}»...')
...
def print_results(results: list[MatchResult], show_initial: bool = False) -> None:
    """Вывести результаты в читаемом виде."""
    print()

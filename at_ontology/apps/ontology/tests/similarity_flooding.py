from __future__ import annotations

import re
from dataclasses import dataclass, field
from collections import defaultdict
from typing import Callable

type PairKey = tuple[str, str]


# ─────────────────────────────────────────────────────────────────────────────
# Структуры данных
# ─────────────────────────────────────────────────────────────────────────────

@dataclass
class MatchResult:
    vertex_a: object
    vertex_b: object
    score: float
    initial_score: float


@dataclass
class _Edge:
    source_id: str
    target_id: str
    label: str


@dataclass
class _Graph:
    vertices: dict[str, object] = field(default_factory=dict)
    edges: list[_Edge] = field(default_factory=list)
    _out_index: dict[str, list[_Edge]] = field(default_factory=dict, repr=False)
    _in_index:  dict[str, list[_Edge]] = field(default_factory=dict, repr=False)

    def _build_indices(self) -> None:
        if self._out_index:
            return
        for vid in self.vertices:
            self._out_index[vid] = []
            self._in_index[vid]  = []
        for e in self.edges:
            if e.source_id in self._out_index:
                self._out_index[e.source_id].append(e)
            if e.target_id in self._in_index:
                self._in_index[e.target_id].append(e)

    def out_edges(self, vertex_id: str) -> list[_Edge]:
        self._build_indices()
        return self._out_index.get(vertex_id, [])

    def in_edges(self, vertex_id: str) -> list[_Edge]:
        self._build_indices()
        return self._in_index.get(vertex_id, [])


@dataclass
class _PCGEdge:
    from_pair: PairKey
    to_pair: PairKey


# ─────────────────────────────────────────────────────────────────────────────
# Шаг 1. Загрузка графа
# ─────────────────────────────────────────────────────────────────────────────

def _get_rel_label(relationship: object) -> str:
    rel_type = getattr(relationship, 'type', None)
    if rel_type is None:
        return 'unknown'
    if hasattr(rel_type, 'alias'):
        return rel_type.alias.rsplit('.', 1)[-1]
    if hasattr(rel_type, 'name'):
        return rel_type.name
    return 'unknown'


def _get_vertex_from_rel_end(rel_end: object) -> object | None:
    if hasattr(rel_end, 'value'):
        return rel_end.value
    return rel_end


def _is_topic(vertex: object) -> bool:
    """Только вершины-темы (Topic_*), компетенции исключаются."""
    return vertex.name.startswith('Topic_')


def _load_graph(
    ontology: object,
    vertex_filter: Callable[[object], bool] | None = None,
) -> _Graph:
    if vertex_filter is None:
        vertex_filter = _is_topic

    g = _Graph()
    for name, vertex in ontology.vertices.items():
        if vertex_filter(vertex):
            g.vertices[name] = vertex

    for _rel_name, rel in ontology.relationships.items():
        try:
            src = _get_vertex_from_rel_end(rel.source)
            tgt = _get_vertex_from_rel_end(rel.target)
        except AttributeError:
            continue
        if src is None or tgt is None:
            continue
        if not vertex_filter(src) or not vertex_filter(tgt):
            continue
        g.edges.append(_Edge(
            source_id=src.name,
            target_id=tgt.name,
            label=_get_rel_label(rel),
        ))
    return g


# ─────────────────────────────────────────────────────────────────────────────
# Шаг 2. Строковая схожесть (только Левенштейн, без аббревиатур)
# ─────────────────────────────────────────────────────────────────────────────

def _levenshtein(a: str, b: str) -> int:
    n, m = len(a), len(b)
    if n == 0: return m
    if m == 0: return n
    prev = list(range(m + 1))
    for i, ch_a in enumerate(a, 1):
        curr = [i] + [0] * m
        for j, ch_b in enumerate(b, 1):
            curr[j] = prev[j-1] if ch_a == ch_b else 1 + min(prev[j], curr[j-1], prev[j-1])
        prev = curr
    return prev[m]


def _str_sim(a: str, b: str) -> float:
    if not a or not b:
        return 0.0
    a, b = a.lower().strip(), b.lower().strip()
    if a == b:
        return 1.0
    return 1.0 - _levenshtein(a, b) / max(len(a), len(b))


def _label_sim(a: str, b: str) -> float:
    """Схожесть двух строк-названий."""
    return _str_sim(a, b)


# ─────────────────────────────────────────────────────────────────────────────
# Шаг 3. Контекстная схожесть через родителя и детей
# ─────────────────────────────────────────────────────────────────────────────

def _get_label(vertex: object) -> str:
    return (getattr(vertex, 'label', None) or vertex.name).strip()


def _get_parent_label(vid: str, graph: _Graph) -> str | None:
    """Label родительской вершины (источник Hierarchy-ребра)."""
    for e in graph._in_index.get(vid, []):
        if e.label == 'Hierarchy':
            parent = graph.vertices.get(e.source_id)
            if parent:
                return _get_label(parent)
    return None


def _get_children_labels(vid: str, graph: _Graph) -> list[str]:
    """Labels дочерних вершин (цели Hierarchy-рёбер)."""
    children = []
    for e in graph._out_index.get(vid, []):
        if e.label == 'Hierarchy':
            child = graph.vertices.get(e.target_id)
            if child:
                children.append(_get_label(child))
    return children


def _children_sim(vid_a: str, vid_b: str,
                  graph_a: _Graph, graph_b: _Graph) -> float:
    """
    Схожесть множеств детей двух вершин.
    Для каждого ребёнка A ищем лучшее совпадение среди детей B,
    усредняем по обоим направлениям.
    """
    ch_a = _get_children_labels(vid_a, graph_a)
    ch_b = _get_children_labels(vid_b, graph_b)

    if not ch_a and not ch_b:
        return 0.5  # оба листа — нейтрально
    if not ch_a or not ch_b:
        return 0.0  # один лист, другой нет — не похожи структурно

    # A→B
    score_ab = sum(max(_label_sim(a, b) for b in ch_b) for a in ch_a) / len(ch_a)
    # B→A
    score_ba = sum(max(_label_sim(b, a) for a in ch_a) for b in ch_b) / len(ch_b)
    return (score_ab + score_ba) / 2.0


def _initial_sigma(
    v_a: object,
    v_b: object,
    graph_a: _Graph,
    graph_b: _Graph,
) -> float:
    """
    σ⁰ = взвешенная комбинация:
      50% — схожесть самих label
      30% — схожесть родителей (контекст раздела)
      20% — схожесть множества детей (структурный контекст)

    Пример почему это работает:
      «Онтология» (родитель: «Общее представление человека об окружающем мире»)
      vs «Основные понятия и определения» (родитель: «Онтологии в многоагентных системах»)
      → схожесть родителей низкая → σ⁰ низкое ✓

      «Онтология» vs «Основные понятия и определения»
      (родитель: «Онтологии в МАС», у которого дети: Модель онтологии, Классификации...)
      → дети пересекаются по смыслу → σ⁰ выше ✓
    """
    vid_a = v_a.name
    vid_b = v_b.name

    label_a = _get_label(v_a)
    label_b = _get_label(v_b)
    self_sim = _label_sim(label_a, label_b)

    parent_a = _get_parent_label(vid_a, graph_a)
    parent_b = _get_parent_label(vid_b, graph_b)

    if parent_a and parent_b:
        par_sim = _label_sim(parent_a, parent_b)
    elif parent_a or parent_b:
        par_sim = 0.2  # один корневой — слабый сигнал
    else:
        par_sim = 0.5  # оба корневые

    child_sim = _children_sim(vid_a, vid_b, graph_a, graph_b)

    return 0.50 * self_sim + 0.30 * par_sim + 0.20 * child_sim


# ─────────────────────────────────────────────────────────────────────────────
# Шаг 4. PCG
# ─────────────────────────────────────────────────────────────────────────────

def _build_pcg(graph_a: _Graph, graph_b: _Graph) -> list[_PCGEdge]:
    edges_b_by_label: dict[str, list[_Edge]] = defaultdict(list)
    for e in graph_b.edges:
        edges_b_by_label[e.label].append(e)

    pcg_edges: list[_PCGEdge] = []
    for e_a in graph_a.edges:
        for e_b in edges_b_by_label.get(e_a.label, []):
            pcg_edges.append(_PCGEdge(
                from_pair=(e_a.source_id, e_b.source_id),
                to_pair=(e_a.target_id, e_b.target_id),
            ))
    return pcg_edges


# ─────────────────────────────────────────────────────────────────────────────
# Шаг 5. Итерации SF — формула C (Melnik et al. 2002)
# ─────────────────────────────────────────────────────────────────────────────

def _normalize_dict(d: dict[PairKey, float]) -> dict[PairKey, float]:
    if not d:
        return d
    max_val = max(d.values())
    if max_val == 0:
        return d
    return {k: v / max_val for k, v in d.items()}


def _flood(
    sigma0: dict[PairKey, float],
    pcg_edges: list[_PCGEdge],
    graph_a: _Graph,
    graph_b: _Graph,
    iterations: int = 10,
    convergence_threshold: float = 1e-4,
    sf_weight: float = 0.3,
) -> dict[PairKey, float]:
    """
    Формула C: σⁱ⁺¹ = normalize(σ⁰ + sf_weight·φ(σⁱ))

    sf_weight контролирует насколько сильно структурные связи
    влияют на итоговый результат. 0.3 = SF добавляет не более 30%
    от текстовой составляющей.
    """
    if not pcg_edges:
        print('  PCG пуст — используем σ⁰')
        return dict(sigma0)

    graph_a._build_indices()
    graph_b._build_indices()

    out_degree_a = {
        vid: max(len(graph_a._out_index.get(vid, [])), 1)
        for vid in graph_a.vertices
    }
    out_degree_b = {
        vid: max(len(graph_b._out_index.get(vid, [])), 1)
        for vid in graph_b.vertices
    }

    incoming: dict[PairKey, list[tuple[PairKey, float]]] = defaultdict(list)
    for e in pcg_edges:
        src_a, src_b = e.from_pair
        w = 1.0 / (out_degree_a.get(src_a, 1) * out_degree_b.get(src_b, 1))
        incoming[e.to_pair].append((e.from_pair, w))

    current = _normalize_dict(dict(sigma0))

    for iteration in range(iterations):
        increments: dict[PairKey, float] = {}
        for pair, sources in incoming.items():
            inc = sum(current.get(src, 0.0) * w for src, w in sources)
            if inc > 0:
                increments[pair] = inc

        # Формула C с ограниченным весом SF
        combined: dict[PairKey, float] = dict(sigma0)
        for pair, inc in increments.items():
            combined[pair] = combined.get(pair, 0.0) + sf_weight * inc

        new_sigma = _normalize_dict(combined)

        delta = sum(
            abs(new_sigma.get(p, 0.0) - current.get(p, 0.0))
            for p in set(new_sigma) | set(current)
        )
        current = new_sigma

        print(f'  Итерация {iteration + 1:2d}: Δ={delta:.6f}')
        if delta < convergence_threshold:
            print(f'  Сошлось за {iteration + 1} итераций')
            break
    else:
        print(f'  Достигнут лимит {iterations} итераций')

    return current


# ─────────────────────────────────────────────────────────────────────────────
# Шаг 6. Фильтрация «один к одному»
# ─────────────────────────────────────────────────────────────────────────────

def _filter_one_to_one(
    sigma: dict[PairKey, float],
    graph_a: _Graph,
    graph_b: _Graph,
    min_score: float = 0.05,
) -> list[tuple[PairKey, float]]:
    sorted_pairs = sorted(sigma.items(), key=lambda x: -x[1])
    used_a: set[str] = set()
    used_b: set[str] = set()
    result: list[tuple[PairKey, float]] = []

    for (vid_a, vid_b), score in sorted_pairs:
        if score < min_score:
            break
        if vid_a not in graph_a.vertices or vid_b not in graph_b.vertices:
            continue
        if vid_a in used_a or vid_b in used_b:
            continue
        used_a.add(vid_a)
        used_b.add(vid_b)
        result.append(((vid_a, vid_b), score))

    return result


# ─────────────────────────────────────────────────────────────────────────────
# Публичный класс
# ─────────────────────────────────────────────────────────────────────────────

class OntologySimilarityMatcher:
    """
    Сопоставление онтологий курсов/дисциплин.

    Алгоритм:
    1. Загружаем только Topic_* вершины (компетенции автоматически исключаются).
    2. σ⁰ = 50% схожесть label + 30% схожесть родителей + 20% схожесть детей.
       Контекст родителя разделяет «Основные понятия» под разными разделами.
       Контекст детей помогает найти «Онтология» ↔ «Основные понятия и определения»
       если их дети семантически близки.
    3. SF (формула C, вес 0.3) слегка корректирует через структуру — не доминирует.
    4. Фильтр «один к одному».

    Параметры
    ---------
    ontology_a, ontology_b : объекты Ontology из at_ontology_parser
    iterations             : макс. итераций SF (по умолчанию 10)
    min_score              : порог отсечения (по умолчанию 0.10)
    sf_weight              : вес структурного распространения (по умолчанию 0.3)
    vertex_filter          : по умолчанию — только Topic_* вершины
    """

    def __init__(
        self,
        ontology_a: object,
        ontology_b: object,
        iterations: int = 10,
        min_score: float = 0.10,
        sf_weight: float = 0.3,
        vertex_filter: Callable[[object], bool] | None = None,
    ) -> None:
        self.ontology_a    = ontology_a
        self.ontology_b    = ontology_b
        self.iterations    = iterations
        self.min_score     = min_score
        self.sf_weight     = sf_weight
        self.vertex_filter = vertex_filter if vertex_filter is not None else _is_topic

    def run(self, top_k: int | None = None) -> list[MatchResult]:
        print('Загружаем граф A...')
        graph_a = _load_graph(self.ontology_a, self.vertex_filter)
        print(f'  {len(graph_a.vertices)} вершин, {len(graph_a.edges)} рёбер')

        print('Загружаем граф B...')
        graph_b = _load_graph(self.ontology_b, self.vertex_filter)
        print(f'  {len(graph_b.vertices)} вершин, {len(graph_b.edges)} рёбер')

        if not graph_a.vertices or not graph_b.vertices:
            print('Одна из онтологий пуста.')
            return []

        graph_a._build_indices()
        graph_b._build_indices()

        print('Строим σ⁰ (label + родитель + дети)...')
        sigma0: dict[PairKey, float] = {}
        for vid_a, v_a in graph_a.vertices.items():
            for vid_b, v_b in graph_b.vertices.items():
                s = _initial_sigma(v_a, v_b, graph_a, graph_b)
                if s > 0:
                    sigma0[(vid_a, vid_b)] = s
        print(f'  {len(sigma0)} пар с σ⁰ > 0')

        print('Строим PCG...')
        pcg_edges = _build_pcg(graph_a, graph_b)
        print(f'  {len(pcg_edges)} рёбер в PCG')

        print(f'Итерации SF (вес={self.sf_weight})...')
        sigma = _flood(
            sigma0, pcg_edges, graph_a, graph_b,
            self.iterations, sf_weight=self.sf_weight,
        )

        print('Фильтрация (один к одному)...')
        filtered = _filter_one_to_one(sigma, graph_a, graph_b, self.min_score)

        results = [
            MatchResult(
                vertex_a=graph_a.vertices[vid_a],
                vertex_b=graph_b.vertices[vid_b],
                score=score,
                initial_score=sigma0.get((vid_a, vid_b), 0.0),
            )
            for (vid_a, vid_b), score in filtered
        ]

        results.sort(key=lambda r: -r.score)
        if top_k is not None:
            results = results[:top_k]

        print(f'Готово. Найдено {len(results)} соответствий.')
        return results


# ─────────────────────────────────────────────────────────────────────────────
# Утилиты
# ─────────────────────────────────────────────────────────────────────────────

def print_results(results: list[MatchResult], show_initial: bool = False) -> None:
    print()
    header = f"{'Вершина A':<35} {'Вершина B':<35} {'σ':>7}"
    if show_initial:
        header += f"  {'σ⁰':>7}"
    print(header)
    print('-' * len(header))

    for r in results:
        name_a = _get_label(r.vertex_a)[:33]
        name_b = _get_label(r.vertex_b)[:33]
        bar  = '█' * int(r.score * 20)
        line = f'{name_a:<35} {name_b:<35} {r.score:>6.3f}'
        if show_initial:
            line += f'  {r.initial_score:>6.3f}'
        print(f'{line}  {bar}')


def debug_pair(label_a: str, label_b: str,
               parent_a: str | None = None, parent_b: str | None = None,
               children_a: list[str] | None = None, children_b: list[str] | None = None) -> None:
    """
    Отладочная функция для проверки σ⁰ конкретной пары без загрузки онтологий.
    """
    self_s = _label_sim(label_a, label_b)
    par_s = _label_sim(parent_a, parent_b) if parent_a and parent_b else 0.5

    ch_a = children_a or []
    ch_b = children_b or []
    if ch_a and ch_b:
        score_ab = sum(max(_label_sim(a, b) for b in ch_b) for a in ch_a) / len(ch_a)
        score_ba = sum(max(_label_sim(b, a) for a in ch_a) for b in ch_b) / len(ch_b)
        ch_s = (score_ab + score_ba) / 2.0
    elif not ch_a and not ch_b:
        ch_s = 0.5
    else:
        ch_s = 0.0

    sigma = 0.50 * self_s + 0.30 * par_s + 0.20 * ch_s
    print(f'A: "{label_a}" (родитель: {parent_a})')
    print(f'B: "{label_b}" (родитель: {parent_b})')
    print(f'  self_sim   = {self_s:.3f}  × 0.50 = {0.50*self_s:.3f}')
    print(f'  parent_sim = {par_s:.3f}  × 0.30 = {0.30*par_s:.3f}')
    print(f'  child_sim  = {ch_s:.3f}  × 0.20 = {0.20*ch_s:.3f}')
    print(f'  σ⁰ = {sigma:.3f}')